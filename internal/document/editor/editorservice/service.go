...
func (s *parserServiceServer) Serialize(_ context.Context, req *parserv1.SerializeRequest) (*parserv1.SerializeResponse, error) {
	...
	for _, cell := range req.Notebook.Cells {
		if req.Identity.Number() == parserv1.RunmeIdentity_RUNME_IDENTITY_ALL.Number() ||
			req.Identity.Number() == parserv1.RunmeIdentity_RUNME_IDENTITY_CELL.Number() {
	...
	if req.Identity.Number() == parserv1.RunmeIdentity_RUNME_IDENTITY_UNSPECIFIED.Number() ||
		req.Identity.Number() == parserv1.RunmeIdentity_RUNME_IDENTITY_CELL.Number() {

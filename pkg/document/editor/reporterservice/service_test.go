...
func TestTransform(t *testing.T) {
	...
	assert.Equal(t, req.AutoSave, &resp.Extension.AutoSave)
	assert.Equal(t, req.Repository, &resp.Extension.Git.Repository)
	assert.Equal(t, req.Branch, &resp.Extension.Git.Branch)
	assert.Equal(t, req.Commit, &resp.Extension.Git.Commit)
	assert.Equal(t, req.FilePath, &resp.Extension.File.Path)
	assert.Equal(t, req.FileContent, resp.Extension.File.Content)
	assert.Equal(t, req.PlainOutput, resp.Extension.Session.PlainOutput)
	assert.Equal(t, req.MaskedOutput, resp.Extension.Session.MaskedOutput)
	assert.Equal(t, req.MacAddress, &resp.Extension.Device.MacAddress)
	assert.Equal(t, req.Hostname, &resp.Extension.Device.Hostname)
	assert.Equal(t, req.Platform, &resp.Extension.Device.Platform)
	assert.Equal(t, req.Release, &resp.Extension.Device.Release)
	assert.Equal(t, req.Arch, &resp.Extension.Device.Arch)
	assert.Equal(t, req.Vendor, &resp.Extension.Device.Vendor)
	assert.Equal(t, req.Shell, &resp.Extension.Device.Shell)
	assert.Equal(t, req.VsAppHost, &resp.Extension.Device.VsAppHost)
	assert.Equal(t, req.VsAppName, &resp.Extension.Device.VsAppName)
	assert.Equal(t, req.VsAppSessionId, &resp.Extension.Device.VsAppSessionId)
	assert.Equal(t, req.VsMachineId, &resp.Extension.Device.VsMachineId)
	assert.Equal(t, req.VsMetadata, resp.Extension.Device.VsMetadata)
}
